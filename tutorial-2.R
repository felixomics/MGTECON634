...
setwd("/Users/thaipham/Desktop/RA-Susan/R Tutorial")
...
linear.train.1 <- model.matrix(linearprop, processed.scaled.train.1)[,-1]
...
# Method 1
# coef(lasso.linear, s = lasso.linear$lambda.1se) 

# Method 2 
# The default for lambda is s = lasso.linear$lambda.1se
...
cv.glm.logit <- cv.glmnet(linear.train.1, y.train[smplcausal,], 
...
visual.pruned.tree <- prune(linear.singletree, cp = 0.01)


plot(visual.pruned.tree, uniform=TRUE,
     main="Visualize The First Few Layers of The Tree")
text(visual.pruned.tree, use.n=TRUE, all=F, cex=.8)
